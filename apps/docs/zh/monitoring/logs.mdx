...
<Warning>
TODO：集中式日志聚合（Datadog、Grafana Loki、AWS CloudWatch）尚未在 Nebutra 中配置。目前，`warn` 和 `error` 日志可在 Sentry 面包屑中查看，而 `info` 和 `debug` 日志只能通过平台的 stdout/日志流获取。完整的日志聚合集成计划在未来版本中实现。
</Warning>
