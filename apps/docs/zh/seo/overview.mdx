...
| robots.txt | 内置 | `apps/landing-page/src/app/robots.ts` |
| XML 网站地图 | 内置 | `apps/landing-page/src/app/sitemap.ts` |
...
结构化数据通过 `<script>` 标签注入到 `apps/landing-page/src/app/[lang]/layout.tsx` 中。根 layout 包含 Organization 和 Website schema。各个页面（如博客文章）可以使用相同模式注入额外的 schema。
