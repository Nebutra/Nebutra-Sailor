...
<Tabs>
  ...
  <Tab title="package">
    ...
    在 `packages/notifications/` 中创建 `package.json`、`tsconfig.json` 和 `src/index.ts`。
  </Tab>
  <Tab title="route">
    在 `apps/api-gateway` 中创建 Hono API 路由：
    ...
    创建 `apps/api-gateway/src/routes/projects.ts`，包含 GET 和 POST 处理器。
  </Tab>
  <Tab title="component">
    ...
    在 `packages/ui/src/components/` 中创建 `pricing-card.tsx` 和 `.stories.tsx`。
  </Tab>
</Tabs>
...
```bash
nebutra i18n sync         # 同步翻译文件
nebutra i18n missing      # 列出缺失的翻译键
...
nebutra preset diff                   # 显示当前与目标预设差异
```
...
nebutra auth switch clerk             # 切换到 Clerk
nebutra auth switch better-auth       # 切换到 Better Auth
```
...
nebutra billing plans list            # 列出已配置的计划
nebutra billing sync                  # 同步计划到 Stripe
```
...
```bash
nebutra ai status                     # 当前 AI 配置
nebutra ai providers list             # 可用服务商列表
nebutra ai switch openai              # 切换默认服务商
```
...
nebutra search reindex --all          # 重建所有索引
```
...
nebutra schema --all                  # 完整 Schema（供 Agent 使用）
nebutra schema --format json          # JSON 格式输出
```
...
```bash
nebutra mcp status                    # MCP 服务器状态
nebutra mcp start                     # 启动 MCP 上下文服务器
```
