...
export function ProductCard({ m, layout = "tile" }: { m: ListingModel; layout?: "tile" | "row" }) {
  ...
  if (layout === "row") {
    return (
      <article className="group overflow-hidden rounded-2xl border border-neutral-6 bg-neutral-1 transition hover:border-neutral-7 hover:shadow-sm">
        <Link href={detailHref} className="flex min-h-[104px] items-stretch">
          ...
          >
            {/* allow-palette: chip surface pinned to match BrandPill tone="light" over cover wash art */}
          ...
          <span className="flex min-w-0 flex-1 flex-col justify-center gap-1.5 px-4 py-3">
            <span className="truncate font-mono text-[13px] font-semibold text-neutral-12">
              {m.publicModel}
            </span>
            <span className="line-clamp-1 text-[12px] text-neutral-10">{desc}</span>
            <span className="text-[11px] text-neutral-9">
  ...
  return (
    <article className="group flex h-full flex-col overflow-hidden rounded-[16px] border border-neutral-6 bg-neutral-1 shadow-sm transition hover:-translate-y-0.5 hover:shadow-md">
      <Link
        href={detailHref}
      ...
      <div className="flex flex-1 flex-col px-3.5 pt-3 pb-3.5">
        <Link href={detailHref} className="min-w-0">
          <p className="truncate font-mono text-[13px] font-semibold text-neutral-12">
            {m.publicModel}
          </p>
          <p className="mt-1 line-clamp-2 min-h-[2.35rem] text-[12px] text-neutral-10" title={desc}>
            {desc}
          </p>
        </Link>
        <div className="mt-2.5 flex items-center gap-1">
          <span className="rounded-md bg-neutral-3 px-1.5 py-0.5 text-[10px] text-neutral-11">
            {CATEGORY_LABEL[m.category]}
          </span>
          <span className="ml-auto flex items-center gap-0.5">
            <Link
              href="/docs"
              className="inline-flex h-6 w-6 items-center justify-center rounded-md text-neutral-10 hover:bg-neutral-3"
              aria-label="文档"
        ...
        <div className="mt-2.5 flex justify-between border-t border-neutral-4 pt-2.5 font-mono text-[11px] text-blue-11">
