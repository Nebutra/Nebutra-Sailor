...
function VisualChip({ children, className }: { children: string; className?: string }) {
  return (
    <span
      className={cn(
        "max-w-[11rem] truncate rounded-full border border-white/10 bg-background/70 px-3 py-1.5 font-mono text-[11px] text-foreground/80 shadow-sm backdrop-blur-md",
        className,
      )}
      translate="no"
    >
...
function VisualStat({ label, value }: { label: string; value: string }) {
  return (
    <div className="rounded-[var(--radius-sm)] border border-white/10 bg-background/55 px-3 py-2 backdrop-blur-md">
      <p className="font-mono text-[18px] text-foreground leading-none" translate="no">
        {value}
      </p>
...
function FloatingOrchestra({ folder }: { folder: CapabilityFolder }) {
  const [core, ...nodes] = folder.topology.nodes;

  return (
    <div className="relative h-[350px] min-h-[350px] overflow-hidden rounded-b-[var(--radius-panel)]">
      <div
        className="absolute left-1/2 top-1/2 size-[21rem] -translate-x-1/2 -translate-y-1/2 rounded-full border border-white/10"
        aria-hidden="true"
      />
      <div
        className="absolute left-1/2 top-1/2 size-[15rem] -translate-x-1/2 -translate-y-1/2 rounded-full border border-primary/25"
        aria-hidden="true"
      />
      <div className="absolute left-1/2 top-1/2 flex h-32 w-52 -translate-x-1/2 -translate-y-1/2 items-center justify-center rounded-[2rem] border border-primary/35 bg-background/80 px-6 text-center shadow-[0_0_80px_rgba(86,120,255,0.24)] backdrop-blur-xl">
        <div>
...
function LayeredStack({ folder }: { folder: CapabilityFolder }) {
  return (
    <div className="relative h-[350px] min-h-[350px] overflow-hidden rounded-b-[var(--radius-panel)] px-7 pt-10">
      ...
      {folder.topology.nodes.slice(0, 5).map((node, index) => (
        <div
          className={cn(
            "absolute left-1/2 flex h-14 w-[78%] -translate-x-1/2 items-center justify-between rounded-[var(--radius-sm)] border border-white/10 bg-background/70 px-5 shadow-2xl backdrop-blur-xl transition-transform duration-300 group-hover/card:-translate-y-1",
            index === 0 && "top-[12%] z-50 border-primary/35 bg-primary/10",
            index === 1 && "top-[27%] z-40 w-[72%]",
            index === 2 && "top-[42%] z-30 w-[66%]",
            index === 3 && "top-[57%] z-20 w-[60%]",
            index === 4 && "top-[72%] z-10 w-[54%]",
          )}
          key={node.label}
        >
...
function Corridor({
  ...
  return (
    <div className="relative h-[350px] min-h-[350px] overflow-hidden rounded-b-[var(--radius-panel)] px-7 pt-12">
      <div
        className="absolute left-12 right-12 top-1/2 h-px bg-gradient-to-r from-transparent via-primary/55 to-transparent"
        aria-hidden="true"
      />
      <div className="grid h-full grid-cols-5 items-center gap-3">
        {folder.topology.nodes.slice(0, 5).map((node, index) => (
          <div className="relative flex flex-col items-center gap-4" key={node.label}>
            <div
              className={cn(
                "flex size-14 items-center justify-center rounded-full border border-white/10 bg-background/75 shadow-xl backdrop-blur-xl",
                index === 0 && "border-primary/35 bg-primary/10",
              )}
            >
              {index === 0 ? (
...
function IntegrationBus({ folder }: { folder: CapabilityFolder }) {
  return (
    <div className="relative h-[350px] min-h-[350px] overflow-hidden rounded-b-[var(--radius-panel)] px-8 pt-10">
      <div className="absolute left-1/2 top-10 h-52 w-44 -translate-x-1/2 rounded-[2rem] border border-primary/30 bg-primary/10 shadow-[0_0_90px_rgba(45,212,191,0.18)] backdrop-blur-xl">
...
function LedgerTable({ folder }: { folder: CapabilityFolder }) {
  return (
    <div className="relative h-[350px] min-h-[350px] overflow-hidden rounded-b-[var(--radius-panel)] px-7 pt-10">
      <div className="mx-auto max-w-lg overflow-hidden rounded-[var(--radius-panel)] border border-white/10 bg-background/70 shadow-2xl backdrop-blur-xl">
...
function CapabilityArtwork({ folder, locale }: { folder: CapabilityFolder; locale: Locale }) {
  ...
  return (
    <div className="relative mt-8 flex-1 overflow-hidden">
      <div
        className={cn(
          "absolute inset-x-4 bottom-0 top-4 rounded-[2rem] bg-gradient-to-br blur-3xl",
      ...
      <div className="relative z-10 mx-6 flex items-center justify-between gap-3 rounded-[var(--radius-sm)] border border-white/10 bg-background/45 px-4 py-3 backdrop-blur-md sm:mx-10">
        <div className="min-w-0">
          <p className="truncate font-semibold text-foreground text-sm">
            {copyFor(folder.topology.title, locale)}
          </p>
          <p className="mt-1 truncate font-mono text-[11px] text-muted-foreground" translate="no">
            {folder.focusPackages.slice(0, 3).join("  /  ")}
          </p>
        </div>
      ...
      <div className="relative z-10">
        {variant === "orchestra" ? <FloatingOrchestra folder={folder} /> : null}
        {variant === "stack" ? <LayeredStack folder={folder} /> : null}
        {variant === "trust" || variant === "supply" || variant === "request" ? (
          <Corridor folder={folder} icon={Icon} />
        ) : null}
        {variant === "bus" ? <IntegrationBus folder={folder} /> : null}
        {variant === "ledger" ? <LedgerTable folder={folder} /> : null}
      </div>
    </div>
  );
}
      ...
      <AnimateInGroup stagger="fast" className="grid grid-cols-1 gap-6 lg:grid-cols-12">
        {CAPABILITY_FOLDERS.map((folder) => {
          ...
          return (
            <article
              className={cn(
                "group/card relative flex min-h-[690px] scroll-mt-28 flex-col overflow-hidden rounded-[var(--radius-panel)] border border-[var(--neutral-6)] bg-background transition-all duration-500 hover:-translate-y-0.5 hover:border-foreground/20 dark:bg-background dark:hover:border-border",
            ...
            >
              ...
              <div className="relative z-10 flex-none px-8 pt-10 sm:px-10">
                ...
                <p className="mt-4 max-w-sm font-medium text-[15px] text-muted-foreground leading-relaxed sm:text-base dark:text-zinc-400">
