...
export function buildSsoLoginUrl(
  ...
  options: { identifier: string; returnUrl: string | null; loginOrigin?: string },
): string {
  const loginUrl =
  ...
  if (provider.provider === "clerk" && options.loginOrigin) {
    return `${options.loginOrigin.replace(/\/$/, "")}${path}`;
  }
  return path;
}
