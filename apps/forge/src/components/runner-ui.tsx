...
const OUTPUT =
  "overflow-x-auto rounded-[var(--radius-lg)] border border-neutral-6 bg-neutral-1 p-3 font-mono text-sm";

const NOTE = "text-xs text-neutral-10";
...
export function RunnerPanel({
  ...
  return (
    <div
      className={["rounded-[var(--radius-lg)] border border-neutral-6 bg-neutral-1 p-4", className]
        .filter(Boolean)
        .join(" ")}
    >
      {title ? <p className="mb-1 text-xs font-medium text-neutral-10">{title}</p> : null}
      {children}
    </div>
  );
}
