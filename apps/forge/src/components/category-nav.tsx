...
export function CategoryNav({ categories }: { categories: readonly string[] }) {
  ...
  return (
    <div className="sticky top-16 z-30 -mx-4 border-b border-neutral-6 bg-neutral-1/90 px-4 py-3 backdrop-blur-md sm:-mx-6 sm:px-6">
      <nav
      ...
      >
        {categories.map((id) => (
          <a
            ...
            href={`#${id}`}
            className={cn(
              "inline-flex h-9 shrink-0 items-center rounded-full border border-neutral-6",
              "bg-neutral-1 px-3 text-sm font-medium text-neutral-11 sm:px-4",
              "transition-colors hover:border-neutral-8 hover:bg-neutral-2 hover:text-neutral-12",
            )}
          >
