...
export function JwtTokenSpecimen({ parts, claimsSummary, note, className }: JwtTokenSpecimenProps) {
  return (
    <div className={cx("space-y-3", className)} data-specimen="jwt-token">
      <div className="flex flex-wrap items-center gap-1 rounded-[var(--radius-lg)] bg-neutral-2 p-3 font-mono text-[0.7rem] leading-relaxed sm:text-xs">
        {parts.map((part, i) => (
          <span key={part.tone} className="inline-flex max-w-full items-center gap-1">
            {i > 0 ? <span className="text-neutral-9">.</span> : null}
            <span
              className={cx(
      ...
      <div className="grid gap-3 md:grid-cols-3">
        {parts.map((part) => (
          <section
            key={part.tone}
            className="rounded-[var(--radius-lg)] bg-neutral-2 p-3"
            aria-label={part.label}
          >
            <div className="mb-2 flex items-center gap-2">
              <span className={cx("h-2 w-2 rounded-full", PART_DOT[part.tone])} aria-hidden />
              <p className="text-xs font-semibold text-neutral-12">{part.label}</p>
            </div>
            <pre className="max-h-48 overflow-auto whitespace-pre-wrap break-all font-mono text-[0.7rem] leading-relaxed text-neutral-11">
              {part.pretty ?? (part.raw || "—")}
            </pre>
          </section>
        ))}
      </div>
      {note ? <p className="text-xs text-neutral-10">{note}</p> : null}
    </div>
  );
}
