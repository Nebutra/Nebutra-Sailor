...
export default async function ForgeHomePage() {
  ...
  return (
    <>
      ...
      <section className="relative w-full overflow-hidden border-b border-neutral-6">
        ...
        <PageFrame className="relative z-10 py-16 text-center md:py-24 lg:py-28">
          ...
          */}
          <div className="mx-auto flex w-full min-w-0 max-w-2xl flex-col items-center gap-5 sm:gap-6">
            <p className="w-full text-xs font-medium tracking-[0.12em] text-neutral-11 uppercase">
              {t("eyebrow")}
            </p>
            <h1
              className="w-full text-balance text-3xl font-semibold text-neutral-12 sm:text-4xl md:text-5xl lg:text-6xl"
              style={{
            ...
            <p className="w-full max-w-xl text-sm leading-relaxed text-neutral-11 sm:text-base md:text-[17px] md:leading-relaxed">
              {t("subtitle")}
            </p>
            <p className="flex w-full flex-wrap items-center justify-center gap-x-3 gap-y-1 text-sm text-neutral-10">
              <span className="tabular-nums">{t("toolCount", { count: hub.tools.length })}</span>
              <span
                className="hidden h-1 w-1 rounded-full bg-neutral-7 sm:inline-block"
                aria-hidden
              />
              <span>{t("apiMcp")}</span>
            </p>
              ...
              <Section key={cat.id} label={label} className="scroll-mt-28 space-y-5 !py-0">
                <div id={cat.id} className="flex items-end justify-between gap-4">
                  <div>
                    ...
                    {hint ? <p className="mt-1 text-sm text-neutral-11">{hint}</p> : null}
                  </div>
                  <span className="text-xs tabular-nums text-neutral-10">{cat.tools.length}</span>
                </div>
