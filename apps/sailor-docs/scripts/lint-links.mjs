...
for (const file of files) {
  ...
  for (let index = 1; index < segments.length; index += 1) {
    const prefix = `/${lang}/${segments.slice(0, index).join("/")}`;
...
for (const route of docsRoutes.keys()) {
  ...
  if (route.startsWith("/en/")) {
    scanned.urls.set(route.replace(/^\/en/, ""), {});
    scanned.urls.set(route.replace(/^\/en/, "/docs"), {});
  }

  for (const lang of supportedLanguages) {
    if (route.startsWith(`/${lang}/`)) {
      scanned.urls.set(route.replace(new RegExp(`^/${lang}`), `/${lang}/docs`), {});
    }
  }
}
...
if (invalidLinks === 0) {
  // biome-ignore lint/suspicious/noConsole: CLI lint script - stdout is expected.
  console.log(`sailor-docs link lint passed (${files.length} files scanned).`);
  process.exit(0);
}

console.warn(
  `::warning::sailor-docs link lint found ${invalidLinks} invalid local links in ${invalidFiles.length} files; keeping advisory to avoid CI noise from existing docs debt.`,
);
