...
The generated types live in `apps/web/src/lib/api/types.generated.ts` and are consumed via `openapi-fetch` from `apps/web/src/lib/api/client.ts` (`getTypedApi` / `browserApiClient`). A stub is committed so the project typechecks before generation; CI overwrites it.
