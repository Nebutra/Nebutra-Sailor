...
The script derives both 11-step scales and prints them as the `colors` block of `brand.config.ts`. Paste it in and run `pnpm brand:apply`: that writes the scales into the token source (`packages/design/design-tokens/tokens/core.json`) and regenerates everything downstream — `styles.css`, the brand metadata, and the values Storybook and the docs display. There is no override stylesheet to layer on top; the source changes, so every surface agrees.
