...
`https://api.yourdomain.com/api/v1/webhooks/clerk`. Add the signing secret as
`CLERK_WEBHOOK_SECRET`. Required events: `user.created`, `user.deleted`,
...
Map each public host to the correct origin (see [Deployment Overview](/deployment/overview)):
marketing/docs → Vercel; `app` / `auth` / `api` → ECS (or your self-host) unless
you have completed an explicit Vercel cutover.
