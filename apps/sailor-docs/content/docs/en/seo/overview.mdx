...
| Robots.txt | Built in | `apps/landing-page/src/app/robots.ts` |
| XML sitemap | Built in | `apps/landing-page/src/app/sitemap.ts` |
...
```typescript apps/landing-page/src/app/[lang]/page.tsx
...
```
apps/landing-page/src/app/[lang]/blog/[slug]/opengraph-image.tsx
```
...
Structured data is injected in `apps/landing-page/src/app/[lang]/layout.tsx` as a `<script>` tag. The root layout includes Organization and Website schemas. Individual pages (e.g., blog posts) can inject additional schemas using the same pattern.
