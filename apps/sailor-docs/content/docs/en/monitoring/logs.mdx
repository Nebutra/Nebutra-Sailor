...
| Sink | Format | Use when |
|------|--------|----------|
| Datadog | JSON | You already use Datadog APM |
| Grafana Loki | JSON | Self-hosted or Grafana Cloud |
| HTTP endpoint | JSON | Custom ingest (Axiom, Better Stack, etc.) |
...
Set up [promtail](https://grafana.com/docs/loki/latest/send-data/promtail/) or use the Grafana Cloud Logs drain:
...
| Log level | Where it appears |
|-----------|-----------------|
| `error` | Sentry breadcrumbs + log drain |
| `warn` | Sentry breadcrumbs + log drain |
| `info` | Log drain only |
| `debug` | Log drain only (not emitted in production by default) |
