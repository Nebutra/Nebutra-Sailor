...
Email template ownership is centralized in `packages/email/src/index.ts`.
...
1. Add or update the public sender helper in `packages/email/src/index.ts`.
...
4. Update `packages/email/src/__tests__/email-contract.test.ts` when the public
   surface changes.
