...
所有变量在启动时由 `@nebutra/config` 进行验证（`packages/config/src/index.ts`）。缺失或为空的必填变量会导致立即退出，并输出描述性错误信息。缺失的可选变量将使用文档中记录的默认值。
...
| 变量 | 是否必填 | 默认值 | 说明 |
|------|----------|--------|------|
| `NEXT_PUBLIC_CLERK_PUBLISHABLE_KEY` | 是 | — | Clerk 可发布密钥。以 `pk_live_` 或 `pk_test_` 开头。 |
| `CLERK_SECRET_KEY` | 是 | — | Clerk 密钥。以 `sk_live_` 或 `sk_test_` 开头。 |
| `CLERK_WEBHOOK_SECRET` | 是 | — | Clerk Webhook 载荷的签名密钥。以 `whsec_` 开头。 |
...
| `EMAIL_FROM` | 是 | — | 所有事务性邮件显示的发件人地址。示例：`Nebutra <noreply@mail.nebutra.com>` |
