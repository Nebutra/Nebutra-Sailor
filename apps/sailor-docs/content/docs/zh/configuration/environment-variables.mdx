...
| `BETTER_AUTH_SECRET` | `AUTH_PROVIDER=better-auth` 时 | — | Better Auth 自托管签名密钥。可用 `openssl rand -base64 32` 生成。 |
| `BETTER_AUTH_URL` | `AUTH_PROVIDER=better-auth` 时 | — | `/api/auth/*` 所在的基础 URL。 |
...
| `AUTH_SSO_DISCOVERY_PROVIDERS` | 启用企业 SSO 发现时 | — | 将邮箱域名映射到 SSO provider 的 JSON 数组。Clerk 条目使用 `/sign-in/sso`; generic 条目必须定义内部 `loginUrl`。 |
