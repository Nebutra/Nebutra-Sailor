...
模型 id 以 [models.dev](https://models.dev) 为准。业务代码优先用 `@nebutra/ai-providers` 的语义档位（`flagship` / `fast`），避免手写过期字符串。
...
| `google/gemini-3.1-pro-preview` | Google | 长上下文旗舰 |
| `google/gemini-3.5-flash` | Google | 快速多模态 |

将完整模型标识符传入 `streamText` / `generateText`。实时目录见 [models.dev](https://models.dev) / [OpenRouter 模型页](https://openrouter.ai/models)。
