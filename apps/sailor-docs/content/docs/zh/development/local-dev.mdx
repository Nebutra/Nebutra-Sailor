...
<Steps>
  ...
  <Step title="生成 Prisma 客户端">
    ...
    pnpm db:generate
    ```
    此命令使用 `packages/db/prisma/schema.prisma` 中的 Schema 运行 `prisma generate`。
  </Step>
  <Step title="执行数据库迁移">
    ```bash
    pnpm db:migrate
  ...
  <Step title="启动开发服务器">
    <Tabs>
      ...
      <Tab title="仅控制台">
        ...
        pnpm dev:dashboard
        ```
        启动 `apps/web`（端口 3000）和 `apps/api-gateway`（端口 3001）。
      </Tab>
      <Tab title="仅营销网站">
        ```bash
        pnpm dev:marketing
...
| `CLERK_SECRET_KEY` | 是 | Clerk 后端 API 密钥 — 从 Clerk 控制台获取 |
| `CLERK_PUBLISHABLE_KEY` | 是 | Clerk 可发布密钥 |
| `STRIPE_SECRET_KEY` | 是 | Stripe 密钥 |
| `STRIPE_WEBHOOK_SECRET` | 是 | Stripe Webhook 签名密钥 |
| `CLERK_WEBHOOK_SECRET` | 是 | Clerk Webhook 签名密钥 |
| `CLICKHOUSE_URL` | 是 | ClickHouse HTTP 接口 URL |
| `AUTH_PROVIDER` | 否 | `clerk`（默认）或 `better-auth` |
...
| 变量 | 是否必填 | 说明 |
|------|---------|------|
| `NEXT_PUBLIC_CLERK_PUBLISHABLE_KEY` | 是 | Clerk 可发布密钥 |
| `CLERK_SECRET_KEY` | 是 | Clerk 后端 API 密钥 |
