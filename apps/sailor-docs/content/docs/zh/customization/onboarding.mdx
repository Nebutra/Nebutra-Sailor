...
<Warning>
  **部分实现：** **欢迎**和**创建组织**步骤已完成并可投入生产。**邀请团队**和**配置**步骤目前仅渲染 UI，不会持久化状态或触发下游操作——提交后直接跳转到仪表板。如果你的产品暂时不需要这些功能，可以直接上线。
</Warning>
...
<Warning>
  欢迎步骤目前在 `apps/web/src/app/(onboarding)/welcome/page.tsx` 中使用内联 SVG。要替换插图，将 `<svg>` 元素换成你自己的资源或 `next/image` 的 `<Image>` 组件即可。集中式资源注册表的正式插图插槽已在规划中，但尚未实现。
</Warning>
