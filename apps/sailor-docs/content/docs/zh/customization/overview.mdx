...
该脚本会推导出两条 11 阶色阶，并以 `brand.config.ts` 中 `colors` 块的形式打印出来。粘贴进去后运行 `pnpm brand:apply`：它会把色阶写入 token 源（`packages/design/design-tokens/tokens/core.json`），并重新生成下游的一切——`styles.css`、品牌元数据，以及 Storybook 和文档展示的数值。不存在叠加在上面的覆盖样式表；改的是源头，所以每个界面都一致。
