                    ...
                    {derivedFilters.map((filter) => (
                        <DataTableFacetedFilter
                            ...
                            onSelectionChange={(value, checked) =>
                                handleFilterChange(filter.id, value, checked)
                            }
                            t={t}
                        />
                    ))}
                        ...
                        <DropdownMenuContent align="end" className="w-56 p-2">
                            {table.getAllLeafColumns().map((column) => {
                                ...
                                return (
                                    ...
                                    >
                                        <span className="flex-1 whitespace-normal break-words leading-tight" title={headerLabel}>{headerLabel}</span>
                                        <span className="flex items-center gap-1">
                                            <button
                                                type="button"
                                                className={`rounded border px-2 py-0.5 text-2xs uppercase transition ${pinLeftActive
                                            ...
                                            <button
                                                type="button"
                                                className={`rounded border px-2 py-0.5 text-2xs uppercase transition ${pinRightActive
                                                    ? "border-blue-600 bg-blue-600 text-white"
                                                    : "border-slate-300 text-slate-500 hover:border-blue-400 hover:text-blue-600"
                                                    }`}
                                                onClick={(event) => {
