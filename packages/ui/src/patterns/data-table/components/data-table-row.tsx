...
function DataTableRow<TData>({
    ...
    return (
        <tr
            ...
            ref={shouldMeasure ? (node) => { if (node) measureElement(node); } : undefined}
            ...
            {row.getVisibleCells().map((cell) => {
                ...
                return (
                    <td
                        ...
                        className={`whitespace-nowrap overflow-hidden text-ellipsis px-4 py-3 align-middle text-sm text-slate-700 dark:text-slate-200 ${isPinned ? "bg-white dark:bg-slate-900" : ""
                            } ${isSelected ? "!bg-blue-100/70 dark:!bg-blue-900/40 outline outline-1 outline-blue-400/50 dark:outline-blue-500/50" : ""
                            }`}
                        style={{
                        ...
                        onMouseEnter={
                            cellSelection
                                ? () => cellSelection.onCellMouseEnter(row.index, columnId)
                                : undefined
                        }
                    >
