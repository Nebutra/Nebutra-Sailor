...
vi.mock("@ai-sdk/anthropic", () => ({
  createAnthropic: vi.fn(() =>
    Object.assign(
      (id: string) => ({ __provider: "anthropic", __id: id }),
...
vi.mock("@ai-sdk/openai", () => ({
  createOpenAI: vi.fn(() =>
    Object.assign(
      (id: string) => ({ __provider: "openai", __id: id }),
      {
        __provider: "openai",
        textEmbeddingModel: vi.fn((id: string) => ({
          __provider: "openai",
...
describe("VercelAIAgent.execute() — fallback wiring", () => {
  ...
  it("does not rotate when the only configured provider succeeds (single-provider deploy)", async () => {
    ...
    const response = await agent.run(
      [{ role: "user", content: "ping", timestamp: new Date() }],
      { tenantId: "t", userId: "u", conversationId: "c" },
    );
