...
export function buildLinear(ctx: CompileContext): BrandPackage {
  ...
  const brand: BrandPackage = {
    ...
    semantic: {
      ...
      popover: tryHexToHsl(ctx.colors.obsidian ?? "#161718", "210 5% 9%"),
      ...
      secondaryForeground: tryHexToHsl(ctx.colors.mist ?? "#d0d6e0", "220 20% 85%"),
      muted: tryHexToHsl(ctx.colors.obsidian ?? "#161718", "210 5% 9%"),
      ...
      info: tryHexToHsl(ctx.colors["signal-teal"] ?? "#02b8cc", "187 98% 40%"),
