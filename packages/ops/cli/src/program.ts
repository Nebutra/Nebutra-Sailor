...
/**
 * Build the fully-configured `nebutra` commander program (every command
 * registered, hooks + help text attached) WITHOUT parsing argv.
 *
 * This is the single source of truth for the CLI surface: `index.ts` parses it
 * at runtime, and `metadata-surface.test.ts` introspects it so `metadata.ts`
 * (the Agent-facing `nebutra schema` surface) can never silently drift from the
 * real registered command set.
 */
export function buildProgram(version: string, isInteractive: boolean): Command {
