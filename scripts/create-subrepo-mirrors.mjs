...
async function getRepo(owner, repo, token) {
  try {
    return await github(`/repos/${owner}/${repo}`, token);
  } catch (error) {
...
async function createRepo(mirror, token) {
  return github(`/orgs/${mirror.owner}/repos`, token, {
    method: "POST",
    body: JSON.stringify({
...
async function updateRepo(mirror, token) {
  await github(`/repos/${mirror.owner}/${mirror.repoName}`, token, {
    method: "PATCH",
    body: JSON.stringify({
      description: mirror.description,
      has_issues: true,
  ...
  await github(`/repos/${mirror.owner}/${mirror.repoName}/topics`, token, {
    method: "PUT",
    body: JSON.stringify({
      names: mirror.topics,
    }),
...
async function main() {
  ...
  if (!token) {
    throw new Error("No GitHub token available. Set SUBREPO_MIRROR_TOKEN/GITHUB_TOKEN/GH_TOKEN or run `gh auth login`.");
  }
