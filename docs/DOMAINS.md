...
| `auth.nebutra.com` | CNAME → `nebutra-auth.nebutra.workers.dev` **proxied** | **Auth edge Worker** (Free-plan safe): `/api/auth/*` + Hyperdrive on CF; UI pass-through to ECS via `resolveOverride`. Not OpenNext. | Rollback: `point-auth-dns.yml` target=ecs; emergency Vercel only |
