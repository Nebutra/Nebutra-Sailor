...
| `auth.nebutra.com` | CNAME → `nebutra-auth.omichiliriku.workers.dev` **proxied** | **Auth edge Worker** (Free ~227 KiB gzip): `/api/auth/*` + Hyperdrive on CF; UI → ECS via `resolveOverride`. Not OpenNext / not Paid. | Rollback: `point-auth-dns.yml` target=ecs; emergency Vercel only |
