...
## Vercel landing-page build failure (commit `5fb7f03`)

Three classes of errors in `pnpm build --filter=@nebutra/landing-page`:

### 1. `"use client"` directive placement bugs (1 file)

**`apps/landing-page/src/components/landing/navbar/DesktopNav.tsx`**
```tsx
...
them directly from source (no build step was run first in the landing-page
build graph).
...
### 3. Transitive effect on landing-page build

Any route importing `@nebutra/license` → `@nebutra/vault` → `./crypto.js`
fails. Currently `apps/landing-page/src/app/api/license/route.ts`.
