...
| **R1 — Editor & render** · cut 1 ✅ 2026-09-07 | **Done:** data-driven section registry (`features/resume/editor/sections.ts`, 16 sections, guard test against the contract), pure reducer, debounced autosave (3 s, 422 → per-field issues), `FieldControl`/`SectionForm`/`ResumeEditor` on `@nebutra/ui/primitives`, compact live preview, `/resume/edit` page, Clerk `proxy.ts` (was missing — R0's `auth()` would have thrown at runtime). **Cut 2 ✅ same day:** one `DocumentModel` serializer (`render/model.ts`) feeding `ResumeDocument` (design / print / ATS) and the Markdown/JSON exporters — a schema change is now one edit, not four; `PreferencesForm` (paper, length, bullets cap, icons, public contact, margins); `/resume/print` with `@page` from preferences (browser print → PDF until R2's Playwright export); preview pane with Card / Document / ATS tabs and `.md` / `.json` downloads; `e2e/sleptons/resume-edit.spec.ts` (unauthenticated gating only). **Left for R1:** section-level PATCH (still full-document PUT), signed-in e2e (needs Clerk test user + DB) | 58 unit tests green; typecheck/lint/`next build` clean |
